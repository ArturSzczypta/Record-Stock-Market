...
while current_date <= end_date:
    ...
    url = 'https://www.gpw.pl/archiwum-notowan-full?type=10&instrument=&date=' + given_date_url
    ...
    try:
        ...
        new, obsolete, relisted = f.check_database(given_df)

        # Record relisted stocks
        f.relist(given_date, relisted)

        # Record delisted stocks
        f.delist(given_date, obsolete)

        # Add new stocks
        f.add(given_df, given_date, new)

        # Write new stock results to separate table for each stock
